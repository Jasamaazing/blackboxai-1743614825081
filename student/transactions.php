...
<div class="bg-white p-6 rounded-lg shadow">
    <h2 class="text-xl font-semibold mb-6">Your Transaction History</h2>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <?php if ($transaction['bottles_inserted'] > 0): ?>
                                <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Deposit</span>
                            <?php else: ?>
                                <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Reward</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <?php if ($transaction['reward_claimed']): ?>
                                Claimed: <?= htmlspecialchars($transaction['reward_name']) ?>
                            <?php else: ?>
                                Bottle Deposit
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium <?= $transaction['bottles_inserted'] > 0 ? 'text-green-600' : 'text-red-600' ?>">
                            <?= $transaction['bottles_inserted'] > 0 ? '+' : '' ?><?= $transaction['bottles_inserted'] ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
